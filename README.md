...
## Непубликувани WordPress чернови

В `_posts/` и `_pages/` има архивирани WordPress чернови и pending страници с `published: false`, `sitemap: false` и `search: false`. Те остават в хранилището за история, но Jekyll не ги включва в `_site`, `sitemap.xml`, `feed.xml` или Lunr индекса.

За да публикувате една от тях:

1. Сменете `published:` на `true` (и премахнете `sitemap: false` / `search: false`, ако искате страницата да влезе в sitemap и търсенето).
2. Прегледайте линковете, картинките и прикачените файлове в текста.
3. Проверете дали категориите и таговете имат съответни страници в `_pages/tax/`.
